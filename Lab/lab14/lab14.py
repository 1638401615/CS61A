...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        possibilities.append(("+[" + code[code_idx] + "]" + edits, cost + 1))
        # Delete
        edits, cost = helper_align(skeleton_idx+1, code_idx)
        possibilities.append(("-[" + skeleton[skeleton_idx] + "]" + edits, cost + 1))
...
def num_splits(s, d):
    ...
    "*** YOUR CODE HERE ***"
    ans = 0
    def helper(first, second, s, d):
        nonlocal ans
        if not s and abs(sum(first) - sum(second)) <= d :
            ans += 1
            return
        elif not s:
            return
        else:
            helper(first + [s[0]], second, s[1:], d)
            helper(first, second + [s[0]], s[1:], d)
    helper([],[],s,d)
    return ans//2
