...
def missing_digits(n):
    ...
    if n < 10:
        ...
        one_digit = n % 10
        ten_digit = (n // 10) % 10
        if one_digit == ten_digit or one_digit == ten_digit + 1:
            return missing_digits(n // 10)
        else:
            return one_digit - ten_digit - 1 + missing_digits(n // 10)
...
def count_change(total):
    ...
    """
    "*** YOUR CODE HERE ***"
    def counter(sum , max_coin):
        if sum == 0:
            return 1
        elif max_coin < 0 or sum < 0:
            return 0
        else:
            return counter(sum - 2**max_coin,max_coin) + counter(sum , max_coin - 1)
    return counter(total,int(total ** 0.5))
