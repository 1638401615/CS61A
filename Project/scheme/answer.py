...
from ast import Expression, arg  # Python 2 compatibility
...
def eval_all(expressions, env):
    ...
    environment ENV and return the value off the last.
    ...
    while expressions is not nil:
        ans = scheme_eval(expressions.first, env, expressions.rest is nil)
        # expr.rest is nil √   True ×
        #
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        function_parameter = target.rest
        Expression = Pair(function_parameter, expressions.rest)
        env.define(function_name, do_lambda_form(Expression, env))
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        env.define(f_name, MacroProcedure(formals, body, env))
        return f_name
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        # BEGIN PROBLEM 19
        "*** YOUR CODE HERE ***"
        """"核心是通过不断创建新的Thunk实例,来储存已经计算的结果,
            这样就不需要再使用frame了
            e.g.计算factorial(3,1) #3的阶乘
            factorial(3,1)->factorial(2,3)->factorial(1,6)
            这时就不需要新的frame,因为结果已经都存在了第二个形参里,直接返回即可
            tail True 表示当前需要进行尾递归
            当目前是尾递归,而且不是原子表达式就返回一个Thunk实例
            本质是改为迭代

            进循环——调用原来的eval——发现tail context——调用新的eval——返回Thunk——结束本次调用,空间回收——继续循环
            他遇到一个表达式,先调用optimized_eval 然后进去的时候，都会得到一个 Thunk,
            对这个东西调用original_scheme_eval,在eval过程中,
            如果遇到tail call就直接返回Thunk,这个时候开始while循环第二遍第三遍等等节省帧,
            如果没有的话就是在调用过一次original_scheme_eval结果的基础上继续original_scheme_eval
        """
