...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        if self.bindings.get(symbol,None) != None:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('the number of formal parameters does not match with the number of argument values')
        if formals is nil and vals is nil:
            ...
            while formals is not nil and vals is not nil:
                ...
                if formals is nil:
                    if vals is not nil:
                        raise SchemeError('the number of formal parameters does not match with the number of argument values')
                elif vals is nil and formals is not nil:
                    raise SchemeError('the number of formal parameters does not match with the number of argument values')
                elif vals is nil and formals is nil:
                    break
        return new_frame
        # END PROBLEM 10
...
def do_and_form(expressions, env):
    ...
    while e.rest is not nil:
        if is_false_primitive(scheme_eval(e.first,env)):
            return False
        e = e.rest
    x = scheme_eval(e.first,env)
    if e is nil:
        return False
    elif is_true_primitive(x):
        return x
    else:
        return False
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    e = expressions
    while e is not nil:
        if is_true_primitive(scheme_eval(e.first,env)):
