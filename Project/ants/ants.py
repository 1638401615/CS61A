...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while current_place != beehive:
            near_bee = rANTdom_else_none(current_place.bees)
            if near_bee != None and self.min_range <= range <= self.max_range:
                break
            near_bee = None
            range += 1
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.digesting > 0:
            ...
            eaten = rANTdom_else_none(self.place.bees)
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.n == 1:
            ...
            while p != None:
                if p.ant:
                    if p.ant.buffed == False:
                        p.ant.damage *= 2
                        p.ant.buffed = True
                    if isinstance(p.ant,ContainerAnt) and p.ant.contained_ant is not None and p.ant.contained_ant.buffed == False:
                        p.ant.contained_ant.damage *= 2
                        p.ant.contained_ant.buffed = True
                p = p.exit
        else:
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        back_dest = self.place.entrance
        if self.scare == True and back_dest != gamestate.beehive:
            self.move_to(back_dest)
            self.last_list[0] -= 1
            if self.last_list[0] == 0:
                self.last_list.pop(0)
                self.status_list.pop(0)
        if self.slow == True:
            self.action = make_slow(self.action,self)
        # END EC
        if self.blocked():
...
def apply_status(status, bee, length):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    bee.status_list.append(status)
    bee.last_list.append(length)
    # END Problem EC
